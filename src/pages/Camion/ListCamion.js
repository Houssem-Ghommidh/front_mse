...
import {fetchcamion,deletecamion} from '../../redux/camionSlice'
...
function ListCamion() {
    ...
    const colors = tokens(theme.palette.mode);
      ...
      const {error} = useSelector(state=>state.camion)
      const {status} = useSelector(state=>state.camion)
        ...
        getActions: ( params ) => 
             ...
             [
              <GridActionsCellItem
                icon={<EditIcon />}
                label="Edit"
  onClick={() =>{navigate(`/gestionnaire/formcamion/${params.id}`)}}
              />,
              <GridActionsCellItem
                icon={<DeleteIcon />}
                label="Delete"
                ...
                onClick={(camion)=> {
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      ...
                      dispatch(deletecamion(params.id))
                    }
                  })
  ...
  return (
    ...
    :camion.getAllData.length===0? "there is no data found":
    <Box> 
    <Box display="flex" justifyContent="space-between" alignItems="center">


<Header title="List des camion" subtitle="Bienvenue a ton liste des camion" />
</Box>
<Box
m="8px 0 0 0"
width="100%"
